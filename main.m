...
% true nu (K-by-d)
nuStar = chol(B)';
if ~CheckS(nuStar,isHomophily,0)
...
for iGraph = gStart:gEnd
    ...
    % region.
    if (~CheckS(muHat, isHomophily, isIdentifiable))
        tmpMuHat = muHat';
        tmpMuHat = tmpMuHat(:);
        tmpMuHat = fmincon(@(x) ProjectObj(x,tmpMuHat), tmpMuHat, ...
            [], [], [], [], -ones(dimLatentPosition*nBlock, 1), ...
            ones(dimLatentPosition*nBlock, 1), @(x) ProjectCon(x, ...
            nBlock, dimLatentPosition, isHomophily, isIdentifiable));
        ...
        for scaleCovariance = 2
            ...
            if exist(savefile, 'file') == 0
                if (modelType == 1) || (scaleCovariance == 5)
                    nConverge = 1000;
                    [errorRate, tau, tauResult] = MC(nVertex, nBlock, ...
                        dimLatentPosition, adjMatrix, muHat, sigmaHat, ...
                        tauHat, rho, tauStar, nuStar, sigmaStar,theta, ...
                        nBurnIn, nConverge, scaleCovariance, modelType,...
                        isHomophily,isIdentifiable);
                else
                    nConverge = 500;
                    [errorRate, tau, tauResult] = MC2(nVertex, nBlock, ...
                        dimLatentPosition, adjMatrix, muHat, sigmaHat,...
                        tauHat, pTauHat, rho, tauStar, nuStar, ...
